...
def calculator_metric(preds, golds, labels):
...
def update_model_weights(model, iterator, optimizer, scheduler, max_grad_norm, gradient_accumulation_steps):
    tr_loss = 0
    model.train()
...
def evaluate(model, iterator, label_map):
    ...
    iob_metric, metric = calculator_metric(preds, golds, label_map)
...
def run(args):
    ...
    for e in range(int(args.num_train_epochs)):
        ...
        tr_loss = update_model_weights(model, train_iterator, optimizer, scheduler, args.max_grad_norm, args.gradient_accumulation_steps)
...
"""
python train.py --data_dir new_data/ --model_name_or_path vinai/phobert-base --output_dir outputs
"""
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name_or_path", default="vinai/phobert-base", type=str, required=True) # vinai/phobert-base
