...
class NerModel(RobertaPreTrainedModel):
    _tied_weights_keys = []
    _keys_to_ignore_on_load_missing = ['ferep', 'lstm', 'classifier']
    ...
    def __init__(self, config, feature=None, use_lstm=False, device="cpu"):
        ...
        if use_lstm:
            self.use_lstm = True
            self.lstm = nn.LSTM(self.hidden_size, config.hidden_size // 2, batch_first=True, bidirectional=True)
            self.hidden_size = config.hidden_size
    ...
    def calculate_loss(self, input_ids, attention_masks, token_masks, segment_ids, label_ids, label_masks, feats):
        active_logits = self(input_ids, attention_masks, token_masks, segment_ids, label_masks, feats)
