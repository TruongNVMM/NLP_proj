...
class NER:
    def __init__(self, pretrain_dir="pretrains/baseline/models",
        ...
        _, self.model, self.feature = model_builder_from_pretrained("vinai/phobert-base",
                                                               num_labels,
                                                               pretrain_dir,
                                                               feat_dir=feat_dir)
    ...
    def convert_sentences_to_features(self, sentences):
        ...
        for sent_id, sentence in enumerate(sentences):
            if self.fe is None:
                ex_words = word_tokenize(sentence, format="text").split()
            else:
            ...
            if self.fe is not None:
                for feat_key, feat_value in self.feature.special_token["[SEP]"]:
                    feat_id = self.feature.feature_infos[feat_key]['label'].index(feat_value) + 1
                    feats[feat_key].append(feat_id)
    ...
    def predict(self, text):
        ...
        for step, batch in enumerate(iterator):
            ...
            for p, w in list(zip(pred, words)):
                label_str = self.label_list[p-1]
                if not label_str == 'O':
                    prefix, label = label_str.split('-')
                    if entity is None:
                        entity = (w, label)
                    else:
                        if entity[-1] == label:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if len(input_text.strip()) == 0:
            ...
            input_text = """ông Trương Ngọc Hân, chủ_tịch UBND tỉnh Đồng_Tháp, tỏ thái độ cương quyết: \"Dứt_khoát không\". ông Trương Ngọc 
            Hân, chủ_tịch UBND tỉnh Đồng_Tháp, tỏ thái độ cương quyết: \"Dứt_khoát không\".""" # 105685
